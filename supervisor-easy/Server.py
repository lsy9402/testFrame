...
class Server(object):
    def __init__(self, host, port, user, password, name=None):
        ...
        self.rpc_proxy = ServerProxy('http://%s:%s@%s:%d/RPC2' % (self.user, self.password, self.host, self.port))
    ...
    def tail_log(self, app_name, format_func):
        ...
        while True:
            _offset = offset
            log, offset, ret = func(app_name, offset, 1024 * 4)
            time.sleep(0.5)
            if log and _offset:
                yield format_func("=" * 100)
